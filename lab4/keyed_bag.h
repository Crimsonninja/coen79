...
//     share no keys.
//     Postcondition: Each item and its key in an object has been added to this keyed_bag.
//
...
namespace coen79_lab4
{
    ...
    public:
        ...
        key_type get_key(const size_type& index) const;
        value_type get_data(const size_type& index) const;
        bool has_key(const key_type& key) const;
        value_type get(const key_type& key) const;
    ...
    // NONMEMBER FUNCTIONS for the keyed_bag class
    keyed_bag operator +(const keyed_bag& b1, const keyed_bag& b2);
    void operator +=(keyed_bag& bag1, const keyed_bag& b2);
}
#endif
